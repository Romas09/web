...
const Promo = ({ show, onHide }) => {
    ...
    const checkPromo = async () => {
        const { data } = await getPromo(name); // Предполагается, что у вас есть функция getPromo, которая возвращает данные промо
        console.log("ID:", );
       if (data[0]!=undefined){return alert('Промо уже существует');}
       else{addPromo()}
    }
